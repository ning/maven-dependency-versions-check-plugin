...
== Use commandline exclusions

Exclusion without an version range can be specified on the commandline to ignore a artifact completely.

Specify one or multiple artifacts separated by comma:

    -Dcheck.exceptions=tablelayout:TableLayout,tablelayout:TableLayout

You can also specify resolved/expected versions as in the XML-Configuration above
    -Dcheck.exceptions=log4j:log4j:1.2.131.2.16

The notation is:
	-Dcheck.exceptions=[groupid:artifactid[:expected-version:resolved-version]][,groupid:artifactid[:expected-version:resolved-version]]*
...
User property is: +check.failBuildInCaseOfConflict+
...
User property is: +check.defaultStrategy+
